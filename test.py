...
#    HTTPS/HTTP 모두 시도, User-Agent 추가, 실패 시 에러 발생
# ——————————————————————————————————————————————
def load_kma_regions():
    ...
    for url in urls:
        try:
            ...
            tree = resp.json()  # JSON 파싱 시도
            print(f"✅ {url} 로딩 성공")
            break
        except Exception as e:
            print(f"⚠️ {url} 로딩 실패: {e}")

    if tree is None:
        raise RuntimeError("KMA 지역 목록을 모두 불러오지 못했습니다. 네트워크나 URL을 확인하세요.")
    ...
    print(f"▶ 로드된 지역 개수: {len(regions)}")
    print(f"▶ '여수' 포함 여부: {'여수' in regions}")
    return regions
